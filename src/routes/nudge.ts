...
const nudgeRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{ Body: NudgeBody }>(
    "/internal/nudge",
    async (request, reply) => {
      ...
      const validTypes: NudgeType[] = ["diet", "workout", "checkin"];
      ...
      const sent = await nudgeAgent.checkAndNudge(
        userId,
        phoneNumber,
        nudgeType,
        mealLabel
