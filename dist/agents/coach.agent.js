...
class CoachAgent {
     ...
     */
    generateProgressionSuggestion(logs, current) {
        if (logs.length === 0) {
     ...
     */
    async analyzeWorkout(userId, currentLog) {
        const logs = await ragService.getLast3Workouts(userId, currentLog.exerciseId);
        const progressionSuggestion = this.generateProgressionSuggestion(logs, currentLog);
        return (`Registrado! ${currentLog.exerciseName}: ${currentLog.weightKg}kg × ${currentLog.reps} × ${currentLog.sets} ` +
    ...
    async analyzeDiet(userId) {
        ...
        const mealsText = summary.mealsLogged.length > 0
            ? summary.mealsLogged.join(", ")
            : "nenhuma refeição registada";
        return (`Hoje: ${Math.round(summary.calories)}kcal / ${Math.round(targetCalories)}kcal | ` +
     ...
     */
    async generateMotivationalCheckIn(userName) {
        const messages = [
