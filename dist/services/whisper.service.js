...
class WhisperService {
    ...
    decryptWhatsAppAudio(encryptedBuffer, mediaKeyBase64) {
    ...
    async transcribeAudio(audioUrl, mediaKey) {
        const response = await fetch(audioUrl);
        const bytes = await response.bytes();
        const raw = Buffer.from(bytes);
        const buffer = mediaKey ? this.decryptWhatsAppAudio(raw, mediaKey) : raw;
